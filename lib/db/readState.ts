import type { TypedSupabaseClient } from "@/actions/utils";
import type { UnreadCounts } from "@/types/actions";
...
export async function getUnreadItemIdsForFeeds(
  ...
  return items.filter((item) => !readItemIds.has(item.id)).map((item) => item.id);
}
