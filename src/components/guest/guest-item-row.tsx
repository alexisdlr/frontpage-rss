...
import { buildGuestReaderHref } from "@/src/lib/guest/routes";
...
export function GuestItemRow({ item, scope }: GuestItemRowProps) {
  const router = useRouter();
  const { markItemRead, setItemReadState } = useGuest();
  ...
  const readerHref = buildGuestReaderHref(item.id, scope);
  ...
  function toggleRead(event: React.MouseEvent) {
    ...
    setItemReadState(item.id, !item.isRead);
  }
