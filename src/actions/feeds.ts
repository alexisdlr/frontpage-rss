...
import { upsertFeedItems } from "@/src/lib/db/syncFeedItems";
import { computeUnreadCounts } from "@/src/lib/db/readState";
import { fetchAndParseFeed } from "@/src/lib/rss";

import type { ActionResult, FeedWithMeta } from "@/src/types/actions";
...
export async function getFeed(
  feedId: string,
): Promise<ActionResult<FeedWithMeta>> {
  const auth = await getAuthenticatedClient();
  if (!auth.ok) return auth;
...
> {
  ...
  const result = await fetchAndParseFeed({
    ...
    consecutiveFailures,
    lastSuccessAt: feed.last_success_at ? new Date(feed.last_success_at) : null,
  });
  ...
  const syncResult = await upsertFeedItems(supabase, feedId, result.feed.items);
